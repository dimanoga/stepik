...
async def convert_banned_words(message: str) -> str:
...
async def wanish_message(message: str) -> str:
    for banned_word in banned_words:
        if banned_word in message:
...
async def check_message(message: dict) -> None:
    role = message["role"]
    message = message["message"]
    if role == 'moderator':
        message = await convert_banned_words(message)
    elif role == 'student':
        message = await wanish_message(message)
    elif role == 'black_list_user':
        message = 'Пользователь забанен, сообщение скрыто'
    elif role == 'admin':
        pass
    else:
        message = 'ERROR_USER_NONE'
...
async def main() -> None:
    ...
    for message in messages:
        tasks.append(asyncio.create_task(check_message(message), name=message["role"]))
