"""
Общее время на дозвон не должно превышать 10 секунд.

Напишите асинхронную функцию call_company(), которая будет имитировать звонок компании.
Функция принимает словарь с информацией о компании и ждет количество секунд, указанное в поле 'call_time': 8,
прежде чем вернуть результат.
Если время дозвона превышает 5 секунд, значит компания не ответила - Обработайте CancelledError в блоке try/except,
чтобы избежать ошибки, или вызовите исключение самостоятельно с помощью raise asyncio.CancelledError().
Если ответили - выведите сообщение об успешном дозвоне, указав имя компании и номер телефона.
Company Company3: 792.763.2559x8121 дозвон успешен
Через 10 секунд отмените все задачи, которые еще не завершились.
Используйте метод task.cancel() для отмены задачи.
"""
import asyncio
