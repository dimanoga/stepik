"""
В рамках подготовки к важной секретной операции вам, как руководителю группы новобранцев в шпионской школе,
поручено провести проверку допуска каждого участника. У вас есть анкеты 20 человек, содержащие всю необходимую
информацию для оценки их готовности участия в операции. Основной критерий — наличие действующего срока доступа,
указанного в поле "Срок доступа" анкеты. Если срок доступа не указан (None), это означает, что доступ к операциям
для данного участника закрыт, и вследствие этого, операция для всей группы должна быть отменена

Кроме того, в анкетах присутствует поле "Уровень секретности", которое определяет время, необходимое для сканирования
профиля участника и преодоления бюрократических препятствий.
...
async def check_access(data_elem: dict[str, str | int]):
    await asyncio.sleep(data_elem["Уровень секретности"])
    if data_elem["Срок доступа"]:
        print(f'Участник {data_elem["Имя"]} {data_elem["Фамилия"]} имеет действующий доступ. '
              f'Продолжительность доступа: {data_elem["Срок доступа"]}')
    else:
        raise ValueError(f'Ошибка доступа: У участника {data_elem["Имя"]} {data_elem["Фамилия"]} '
                         f'срок доступа истек или не указан.')


async def main():
    tasks = [asyncio.create_task(check_access(data_elem),
                                 name=f'{data_elem["Имя"]} {data_elem["Фамилия"]}') for data_elem in data]
