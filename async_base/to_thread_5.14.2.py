...
def find_most_common_word(text: str) -> str:
    return Counter(text.lower().split()).most_common()[0][0]


async def download_and_process(article: dict):
    await asyncio.sleep(random.randint(1, 5) / 10)
    most_common_word = await asyncio.to_thread(find_most_common_word, article['content'])
    article['tag'] = most_common_word
